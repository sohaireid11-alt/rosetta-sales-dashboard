...
type Stage = (typeof STATUSES)[number];
type SalesRecord = {
  id: number;
  leadName: string;
  company: string;
...
function emptyRecordForm(): RecordForm {
  return {
    leadName: "", company: "", organizationType: "Individual", sourceType: "Direct enquiry", referredBy: "", requestReceivedBy: "Admin",
    service: "Scheduled Interpretation", serviceDelivery: "In-person", interpretationMode: "Consecutive", opportunityType: "One-time project", stage: "New",
    contactName: "", contactTitle: "", contactEmail: "", contactPhone: "", meetingStage: "No meeting yet", nextMeetingAt: "", nextFollowUpAt: "", nextAction: "",
    dealValue: "", createdAt: today(), closedAt: "", initialNote: "",
  };
}

function emptyClientFollowUpForm(): ClientFollowUpForm {
  return {
    salesRecordId: "", clientName: "", relationshipType: "Recurring client", lastEngagementAt: "", lastCheckInAt: "", satisfactionStatus: "Healthy",
...
function legacySource(value: string) {
  if (value === "Referral" || value === "Repeat client") return "External client referral";
  return SOURCE_TYPES.includes(value as (typeof SOURCE_TYPES)[number]) ? value : "Other";
}

function legacyService(value: string) {
  if (value === "Translation" || value === "Localization") return "Translation and Proofreading of Documents";
  if (value === "Interpretation") return "Scheduled Interpretation";
  if (value === "Training" || value === "Other") return "Other service";
  return SERVICES.includes(value as (typeof SERVICES)[number]) ? value : "Other service";
}

function legacyStatus(value: string) {
  if (value === "Qualified" || value === "Proposal") return "Pending";
  return STATUSES.includes(value as Stage) ? value as Stage : "New";
}

function csvRecords(content: string): RecordForm[] {
  ...
  return rows.slice(1).map((row) => {
    const service = legacyService(cell(row, indexes.service));
    return {
      leadName: cell(row, indexes.leadName), company: cell(row, indexes.company),
      organizationType: cell(row, indexes.organizationType) || "Individual", sourceType: legacySource(cell(row, indexes.sourceType)),
      referredBy: cell(row, indexes.referredBy), requestReceivedBy: cell(row, indexes.requestReceivedBy) || "Admin", service,
      serviceDelivery: cell(row, indexes.serviceDelivery) || (service === "Scheduled Interpretation" ? "In-person" : ""),
      interpretationMode: cell(row, indexes.interpretationMode) || (service === "Scheduled Interpretation" ? "Consecutive" : ""),
      opportunityType: cell(row, indexes.opportunityType) || "One-time project", stage: legacyStatus(cell(row, indexes.stage)),
      ...
      meetingStage: cell(row, indexes.meetingStage) || "No meeting yet", nextMeetingAt: cell(row, indexes.nextMeetingAt), nextFollowUpAt: cell(row, indexes.nextFollowUpAt),
...
export default function Home() {
  ...
  async function loadWorkspace() {
    ...
    try {
      const [recordsResponse, teamResponse, clientResponse] = await Promise.all([
        fetch("/api/deals", { cache: "no-store" }), fetch("/api/team-members", { cache: "no-store" }), fetch("/api/client-follow-ups", { cache: "no-store" }),
      ]);
  ...
  async function loadActivities(record: SalesRecord) {
    setActivityRecord(record); setActivities([]); setActivityText(""); setActivityType("Note"); setError("");
    try {
  ...
  const performance = useMemo(() => {
    const won = visibleRecords.filter((record) => record.stage === "Won");
    const active = visibleRecords.filter((record) => !["Won", "Lost", "Canceled"].includes(record.stage));
    const closed = visibleRecords.filter((record) => ["Won", "Lost", "Canceled"].includes(record.stage));
    ...
    visibleRecords.forEach((record) => {
      ...
      row.leads += 1;
      if (record.stage === "Won") { row.won += 1; row.revenue += record.bookedRevenueCents; }
      if (!["Won", "Lost", "Canceled"].includes(record.stage)) row.pipeline += record.estimatedRevenueCents;
  ...
  const careNeedingAttention = useMemo(() => clientFollowUps.filter((item) => ["Needs attention", "At risk"].includes(item.satisfactionStatus)).length, [clientFollowUps]);
  ...
  function openNewRecord() { setEditingRecord(null); setForm(emptyRecordForm()); setNotice(""); setError(""); setIsFormOpen(true); }
  ...
  async function importCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const importedRows = csvRecords(await file.text());
  ...
  function openNewClientFollowUp() { setEditingClientFollowUp(null); setClientForm(emptyClientFollowUpForm()); setError(""); setIsClientFormOpen(true); }
  ...
  const pageTitle = activeView === "overview" ? "Sales performance" : activeView === "records" ? "Sales records" : "Client care";
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand-lockup"><img src="/rosetta-logo-horizontal.png" alt="Rosetta Languages" /><span className="brand-divider" aria-hidden="true" /><span className="product-name">Sales performance</span></div><div className="topbar-actions">{access.configured ? <><a className="secondary-action" href="/team">Team access</a><form action="/api/auth/logout" method="post"><button className="secondary-action" type="submit">Sign out</button></form></> : null}</div></header>
    ...
    <div className="workspace">
      <nav className="section-tabs" aria-label="Dashboard sections"><button type="button" className={activeView === "overview" ? "tab is-active" : "tab"} onClick={() => setActiveView("overview")}>Overview</button><button type="button" className={activeView === "records" ? "tab is-active" : "tab"} onClick={() => setActiveView("records")}>Sales records <span className="tab-count">{records.length}</span></button><button type="button" className={activeView === "client-care" ? "tab is-active" : "tab"} onClick={() => setActiveView("client-care")}>Client care <span className="tab-count">{clientFollowUps.length}</span></button></nav>
      <section className="page-heading"><div><p className="eyebrow">Rosetta Languages</p><h1>{pageTitle}</h1><p className="heading-copy">{pageDescription}</p></div><button type="button" className="primary-action" onClick={activeView === "client-care" ? openNewClientFollowUp : openNewRecord}><span aria-hidden="true">+</span>{activeView === "client-care" ? "Add client follow-up" : "Add sales record"}</button></section>
        ...
        <section className="records-section records-table">
          <div className="table-wrap"><table>
            ...
            <tbody>{visibleRecords.map((record) => <tr key={record.id}>
              ...
              <td>{record.service}{record.service === "Scheduled Interpretation" ? <small>{record.serviceDelivery}{" \u00b7 "}{record.interpretationMode}</small> : null}</td>
              <td><span className={stageClass(record.stage)}>{record.stage}</span><small>{record.meetingStage}</small></td>
              <td><span className={`follow-up-tag follow-up-${followUpState(record.nextFollowUpAt)}`}>{followUpLabel(record)}</span><small>{record.nextAction || "Action not set"}{" \u00b7 "}{dateLabel(record.nextFollowUpAt)}</small></td>
              <td>{record.sourceType}<small>{record.referredBy || record.requestReceivedBy}</small></td>
              <td className="revenue-cell">{money(record.estimatedRevenueCents)}<small>{record.opportunityType}</small></td>
      ...
      {!isLoading && activeView === "client-care" ? <section className="records-section client-care-section"><div className="client-care-heading"><div><p className="eyebrow">Post-conversion pipeline</p><h2>Client satisfaction follow-ups</h2><p className="table-note">Keep a visible cadence with clients and note the next opportunity to support them.</p></div></div><div className="table-wrap"><table><thead><tr><th>Client</th><th>Relationship</th><th>Satisfaction</th><th>Last check-in</th><th>Next action</th><th>Next follow-up</th><th aria-label="Actions" /></tr></thead><tbody>{clientFollowUps.map((item) => <tr key={item.id}><td><strong>{item.clientName}</strong><small>{item.linkedLeadName ? `Linked to ${item.linkedLeadName}` : "Not linked to a sales record"}</small></td><td>{item.relationshipType}<small>Last service: {dateLabel(item.lastEngagementAt)}</small></td><td><span className={`care-status care-${item.satisfactionStatus.toLowerCase().replaceAll(" ", "-")}`}>{item.satisfactionStatus}</span></td><td>{dateLabel(item.lastCheckInAt)}</td><td>{item.nextAction || "Action not set"}<small>{item.expansionOpportunity || "No expansion note"}</small></td><td><span className={`follow-up-tag follow-up-${followUpState(item.nextFollowUpAt)}`}>{dateLabel(item.nextFollowUpAt)}</span></td><td><div className="row-actions"><button type="button" onClick={() => openEditClientFollowUp(item)}>Edit</button><button type="button" className="delete-button" onClick={() => void deleteClientFollowUp(item)}>Delete</button></div></td></tr>)}</tbody></table>{!clientFollowUps.length ? <p className="empty-table">Add a converted client to begin the satisfaction follow-up pipeline.</p> : null}</div></section> : null}
    </div>

    {isFormOpen ? <div className="modal-backdrop" role="presentation"><form className="record-modal" onSubmit={saveRecord}><div className="modal-heading"><div><p className="eyebrow">Lead workspace</p><h2>{editingRecord ? "Update sales record" : "Add sales record"}</h2></div><button type="button" className="icon-button" aria-label="Close form" onClick={() => setIsFormOpen(false)}>×</button></div><div className="form-section"><h3>Lead and referral</h3><div className="form-grid"><label>Lead name<input required value={form.leadName} onChange={(event) => setForm({ ...form, leadName: event.target.value })} placeholder="Person or organization" /></label><label>Company <span className="optional">Optional</span><input value={form.company} onChange={(event) => setForm({ ...form, company: event.target.value })} placeholder="Organization name" /></label><label>Organization type<select value={form.organizationType} onChange={(event) => setForm({ ...form, organizationType: event.target.value })}>{ORGANIZATION_TYPES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Request received by<select value={form.requestReceivedBy} onChange={(event) => setForm({ ...form, requestReceivedBy: event.target.value })}>{requestReceivedOptions.map((option) => <option key={option}>{option}</option>)}</select></label><label>Source type<select value={form.sourceType} onChange={(event) => setForm({ ...form, sourceType: event.target.value })}>{SOURCE_TYPES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Referred by <span className="optional">Optional</span><input value={form.referredBy} onChange={(event) => setForm({ ...form, referredBy: event.target.value })} placeholder="Person, client, or partner" /></label></div></div><div className="form-section"><h3>Contact person</h3><div className="form-grid"><label>Full name <span className="optional">Optional</span><input value={form.contactName} onChange={(event) => setForm({ ...form, contactName: event.target.value })} placeholder="Main contact" /></label><label>Title <span className="optional">Optional</span><input value={form.contactTitle} onChange={(event) => setForm({ ...form, contactTitle: event.target.value })} placeholder="Role or department" /></label><label>Email <span className="optional">Optional</span><input type="email" value={form.contactEmail} onChange={(event) => setForm({ ...form, contactEmail: event.target.value })} placeholder="name@organization.org" /></label><label>Phone <span className="optional">Optional</span><input value={form.contactPhone} onChange={(event) => setForm({ ...form, contactPhone: event.target.value })} placeholder="Phone number" /></label></div></div><div className="form-section"><h3>Service and opportunity</h3><div className="form-grid"><label>Service<select value={form.service} onChange={(event) => setForm({ ...form, service: event.target.value, serviceDelivery: event.target.value === "Scheduled Interpretation" ? form.serviceDelivery || "In-person" : "", interpretationMode: event.target.value === "Scheduled Interpretation" ? form.interpretationMode || "Consecutive" : "" })}>{SERVICES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Opportunity type<select value={form.opportunityType} onChange={(event) => setForm({ ...form, opportunityType: event.target.value })}>{OPPORTUNITY_TYPES.map((option) => <option key={option}>{option}</option>)}</select></label>{form.service === "Scheduled Interpretation" ? <><label>Delivery<select value={form.serviceDelivery} onChange={(event) => setForm({ ...form, serviceDelivery: event.target.value })}>{INTERPRETATION_DELIVERIES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Interpretation mode<select value={form.interpretationMode} onChange={(event) => setForm({ ...form, interpretationMode: event.target.value })}>{INTERPRETATION_MODES.map((option) => <option key={option}>{option}</option>)}</select></label></> : null}<label>Status<select value={form.stage} onChange={(event) => setForm({ ...form, stage: event.target.value as Stage })}>{STATUSES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Deal value (USD)<input required inputMode="decimal" type="number" min="0" step="0.01" value={form.dealValue} onChange={(event) => setForm({ ...form, dealValue: event.target.value })} placeholder="0" /></label><label>Lead date<input required type="date" value={form.createdAt} onChange={(event) => setForm({ ...form, createdAt: event.target.value })} /></label><label>Close date <span className="optional">Optional</span><input type="date" value={form.closedAt} onChange={(event) => setForm({ ...form, closedAt: event.target.value })} /></label></div></div><div className="form-section"><h3>Meetings and next step</h3><div className="form-grid"><label>Meeting stage<select value={form.meetingStage} onChange={(event) => setForm({ ...form, meetingStage: event.target.value })}>{MEETING_STAGES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Next meeting date <span className="optional">Optional</span><input type="date" value={form.nextMeetingAt} onChange={(event) => setForm({ ...form, nextMeetingAt: event.target.value })} /></label><label>Date of next follow-up{form.stage === "Pending" ? null : <span className="optional"> Optional</span>}<input required={form.stage === "Pending"} type="date" value={form.nextFollowUpAt} onChange={(event) => setForm({ ...form, nextFollowUpAt: event.target.value })} /></label><label>Action needed{form.stage === "Pending" ? null : <span className="optional"> Optional</span>}<select required={form.stage === "Pending"} value={form.nextAction} onChange={(event) => setForm({ ...form, nextAction: event.target.value })}><option value="">Select next action</option>{FOLLOW_UP_ACTIONS.map((option) => <option key={option}>{option}</option>)}</select></label>{editingRecord ? <div className="wide-field note-section"><div><h3>Activity notes</h3><p className="table-note">Each note is saved as its own dated box.</p></div><button type="button" className="secondary-action" onClick={() => void loadActivities(editingRecord)}>Add Note</button></div> : <label className="wide-field">First activity note <span className="optional">Optional</span><textarea rows={3} value={form.initialNote} onChange={(event) => setForm({ ...form, initialNote: event.target.value })} placeholder="A separate dated activity is created; previous notes remain untouched." /></label>}</div></div>{error ? <p className="form-error" role="alert">{error}</p> : null}<div className="modal-actions"><button type="button" className="secondary-action" onClick={() => setIsFormOpen(false)}>Cancel</button><button type="submit" className="primary-action" disabled={isSaving}>{isSaving ? "Saving..." : editingRecord ? "Save changes" : "Add sales record"}</button></div></form></div> : null}
    {activityRecord ? <div className="modal-backdrop" role="presentation"><section className="record-modal activity-modal"><div className="modal-heading"><div><p className="eyebrow">Activity history</p><h2>{activityRecord.leadName}</h2></div><button type="button" className="icon-button" aria-label="Close activity history" onClick={() => setActivityRecord(null)}>×</button></div><form className="activity-form" onSubmit={addActivity}><label>Activity type<select value={activityType} onChange={(event) => setActivityType(event.target.value as (typeof ACTIVITY_TYPES)[number])}>{ACTIVITY_TYPES.map((option) => <option key={option}>{option}</option>)}</select></label><label>New note<textarea required rows={3} value={activityText} onChange={(event) => setActivityText(event.target.value)} placeholder="What happened, what was sent, or what was agreed?" /></label><button type="submit" className="primary-action" disabled={isAddingActivity}>{isAddingActivity ? "Adding..." : "Add Note"}</button></form>{error ? <p className="form-error" role="alert">{error}</p> : null}<div className="activity-list">{activities.length ? activities.map((activity) => <article className="activity-item" key={activity.id}><div><span className="activity-type">{activity.activityType}</span><time>{dateTimeLabel(activity.createdAt)}</time></div><p>{activity.content}</p></article>) : <p className="empty-copy">No activity yet. Add the first update above.</p>}</div></section></div> : null}
    {isClientFormOpen ? <div className="modal-backdrop" role="presentation"><form className="record-modal" onSubmit={saveClientFollowUp}><div className="modal-heading"><div><p className="eyebrow">Client care</p><h2>{editingClientFollowUp ? "Update client follow-up" : "Add client follow-up"}</h2></div><button type="button" className="icon-button" aria-label="Close form" onClick={() => setIsClientFormOpen(false)}>×</button></div><div className="form-section"><h3>Relationship</h3><div className="form-grid"><label>Linked won lead <span className="optional">Optional</span><select value={clientForm.salesRecordId} onChange={(event) => { const selected = records.find((record) => record.id === Number(event.target.value)); setClientForm({ ...clientForm, salesRecordId: event.target.value, clientName: selected ? selected.leadName : clientForm.clientName }); }}><option value="">Select a won lead</option>{records.filter((record) => record.stage === "Won").map((record) => <option key={record.id} value={record.id}>{record.leadName}</option>)}</select></label><label>Client name<input required value={clientForm.clientName} onChange={(event) => setClientForm({ ...clientForm, clientName: event.target.value })} placeholder="Client or organization" /></label><label>Relationship type<select value={clientForm.relationshipType} onChange={(event) => setClientForm({ ...clientForm, relationshipType: event.target.value })}>{RELATIONSHIP_TYPES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Last service date <span className="optional">Optional</span><input type="date" value={clientForm.lastEngagementAt} onChange={(event) => setClientForm({ ...clientForm, lastEngagementAt: event.target.value })} /></label></div></div><div className="form-section"><h3>Satisfaction check-in</h3><div className="form-grid"><label>Satisfaction status<select value={clientForm.satisfactionStatus} onChange={(event) => setClientForm({ ...clientForm, satisfactionStatus: event.target.value })}>{SATISFACTION_STATUSES.map((option) => <option key={option}>{option}</option>)}</select></label><label>Last satisfaction check-in <span className="optional">Optional</span><input type="date" value={clientForm.lastCheckInAt} onChange={(event) => setClientForm({ ...clientForm, lastCheckInAt: event.target.value })} /></label><label>Next follow-up date <span className="optional">Optional</span><input type="date" value={clientForm.nextFollowUpAt} onChange={(event) => setClientForm({ ...clientForm, nextFollowUpAt: event.target.value })} /></label><label>Next action <span className="optional">Optional</span><select value={clientForm.nextAction} onChange={(event) => setClientForm({ ...clientForm, nextAction: event.target.value })}><option value="">Select next action</option>{FOLLOW_UP_ACTIONS.map((option) => <option key={option}>{option}</option>)}</select></label><label className="wide-field">Expansion opportunity <span className="optional">Optional</span><textarea rows={3} value={clientForm.expansionOpportunity} onChange={(event) => setClientForm({ ...clientForm, expansionOpportunity: event.target.value })} placeholder="Potential next service, renewal, referral, or expansion." /></label></div></div>{error ? <p className="form-error" role="alert">{error}</p> : null}<div className="modal-actions"><button type="button" className="secondary-action" onClick={() => setIsClientFormOpen(false)}>Cancel</button><button type="submit" className="primary-action" disabled={isSaving}>{isSaving ? "Saving..." : editingClientFollowUp ? "Save changes" : "Add client follow-up"}</button></div></form></div> : null}
  </main>;
}
